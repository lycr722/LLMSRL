...
class LAMRec(BaseModel):
    def __init__(self, dataset: SampleEHRDataset, embedding_dim: int, bce_weight: float = 0.5, **kwargs):
        ...
        if self.use_llm:
            ...
            self.precomputed_conditions = self.load_precomputed_embeddings(
                r"D:\Code\LAMRec-RAGBK\models\MIMIC3_DIAGNOSES_Embeddings_512_ID.csv", "ICD9_CODE")
            self.precomputed_procedures = self.load_precomputed_embeddings(
                r"D:\Code\LAMRec-RAGBK\models\MIMIC3_PROCEDURES_Embeddings_512_ID.csv", "ICD9_CODE")
            print("正在加载药物语义嵌入...")
            self.precomputed_drugs_semantic = self.load_semantic_embeddings(
                r"D:\Code\LAMRec-RAGBK\models\MIMIC3_Drug_Embeddings_768_ID.csv", # <-- 语义文件路径
                target_dim=self.semantic_dim
            )
            # self.precomputed_drugs_semantic = self.load_semantic_embeddings(
            #     r"D:\Code\LAMRec-RAGBK\models\filtered_drug_embeddings_768.csv", # <-- 语义文件路径
            #     target_dim=self.semantic_dim
            # )
            print("正在加载药物分子嵌入...")
            self.precomputed_drugs_molecular = self.load_molecular_embeddings(
                r"D:\Code\LAMRec-RAGBK\models\aligned_drugs_and_smiles_features_768.csv", # <-- 分子文件路径
    ...
    def generate_ddi_adj(self) -> torch.tensor:
        ddi_path = r"D:\Code\LAMRec-RAGBK\ddi_A_final.pkl"
        voc_path = r"D:\Code\LAMRec-RAGBK\mimic-iii-1.4\zip\voc_final.pkl"

        with open(ddi_path, "rb") as f:
            ddi_adj = np.asarray(dill.load(f), dtype=np.float32)

        with open(voc_path, "rb") as f:
            voc = dill.load(f)

        med_voc = voc["med_voc"]
        ddi_order = [med_voc.idx2word[i] for i in range(len(med_voc.idx2word))]
        label_order = [
            self.label_tokenizer.vocabulary.idx2token[i]
            for i in range(self.label_tokenizer.get_vocabulary_size())
        ]

        if set(ddi_order) != set(label_order):
            missing = sorted(set(label_order) - set(ddi_order))
            extra = sorted(set(ddi_order) - set(label_order))
            raise ValueError(
                "DDI vocabulary does not match the model label tokenizer. "
                f"missing_in_ddi={missing}, extra_in_ddi={extra}"
            )

        perm = [ddi_order.index(code) for code in label_order]
        ddi_adj = ddi_adj[np.ix_(perm, perm)]
        ddi_adj = np.maximum(ddi_adj, ddi_adj.T)
        np.fill_diagonal(ddi_adj, 0)

        print(
            "Loaded DDI adjacency from ddi_A_final.pkl and reordered it to "
            f"the current label tokenizer order. shape={ddi_adj.shape}, "
            f"undirected_edges={int(np.triu(ddi_adj, 1).sum())}"
        )
        return torch.tensor(ddi_adj, dtype=torch.float32)

    def forward(self, conditions: List[List[List[str]]], procedures: List[List[List[str]]], drugs: List[List[str]], drugs_hist: List[List[str]],
        ...
        if self.use_llm:
            ...
            stacked_drug_embs = torch.stack([semantic_fused_emb, molecular_fused_emb], dim=1)

            # 步骤 4.2: 通过MLP-Mixer进行模态交互
            interacted_embs = self.drug_fusion_mixer(stacked_drug_embs)

            # 步骤 4.3: 通过注意力门控进行最终融合
            drugs_emb = self.drug_fusion_gate(interacted_embs)
        ...
        return {
            ...
            "y_pred_indices": y_pred_indices,  # 新增
            "y_true_indices": y_true_indices,  # 新增
        }
