...
class LAMRec(BaseModel):
    def __init__(self, dataset: SampleEHRDataset, embedding_dim: int, bce_weight: float = 0.5, **kwargs):
        ...
        if self.use_llm:
            ...
            self.precomputed_conditions = self.load_precomputed_embeddings(
                r"D:\Code\LAMRec-RAGBK\models\DIAGNOSES_Official_Embedding_512_large.csv", "ICD9_CODE")
            self.precomputed_procedures = self.load_precomputed_embeddings(
                r"D:\Code\LAMRec-RAGBK\models\PROCEDURES_Official_Embedding_512_large.csv", "ICD9_CODE")
            print("正在加载药物语义嵌入...")
            # self.precomputed_drugs_semantic = self.load_semantic_embeddings(
            #     r"D:\Code\LAMRec-RAGBK\models\MIMIC3_Drug_Embeddings_768_HCLFinal_RAG.csv", # <-- 语义文件路径
    ...
    def generate_ddi_adj(self) -> torch.tensor:
        file_path = r"D:\Code\LAMRec-RAGBK\models\ddi_adj_final_131.csv"
    ...
    def get_external_embedding(self,
        ...
        # 检查是否为极端冷启动 Batch (整个 Batch 都没有历史用药)
        is_extreme_cold_start = not codes_no_padding or not any(codes_no_padding)

        if is_extreme_cold_start:
            print("\n" + "!" * 50)
            print("【冷启动实验证据捕获】")
            print("触发条件: 当前 Batch 内所有患者均为初诊患者，无历史用药记录。")
            print(f"该 Batch 原始用药历史数据样例 (patients_data): {patients_data[:3]} ... (全为 <pad> 或空)")
            print("处理机制: 模型已自动将这些缺失模态初始化为全零向量，依靠当前诊断进行推荐。")
            print("!" * 50 + "\n")

        # 步骤 2: 修复冷启动崩溃，强制 max_codes 至少为 1
        max_codes = max(len(codes) for codes in codes_no_padding) if not is_extreme_cold_start else 1
