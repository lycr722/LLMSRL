...
DRUG_PROMPT_TMPL = (
    ...
    "identified by its ATC code. This description must bridge its clinical application with its "
    ...
    "For the medication with ATC code: {atc_code} "
    ...
    "ICD-9-CM: [code] and ATC: [code].\n"
    "1. State the broader anatomical main group and therapeutic context.\n"
    "2. Describe the typical diseases it treats with ICD-9-CM diagnosis codes and associated clinical procedures "
    "with ICD-9-CM procedure codes when applicable.\n"
    "3. Detail the pharmacological class, mechanism of action, common synergistic co-prescriptions with ATC codes, "
    "and critical drug-drug interactions.\n"
)
...
DIAGNOSIS_PROMPT_TMPL = (
    ...
    "3. Major Contraindications: Specify drugs or drug classes with ATC codes that should be avoided "
    ...
    "What first-line and second-line drug classes with ATC codes are typically prescribed?\n"
)
...
PROCEDURE_PROMPT_TMPL = (
    ...
    "Never use quotation marks for any terms, names, or codes. "
    "Do not use bullet points, headings, or numbered lists. "
    "Embed all medical codes naturally using ICD-9-CM: [code] and ATC: [code].\n"
    "1. Procedure definition and purpose.\n"
    "2. Primary indications with ICD-9-CM diagnosis codes.\n"
    "3. Pre-procedure medication adjustments with ATC codes.\n"
    "4. Post-procedure medications with ATC codes.\n"
...
def check_api_key():
    if not DEEPSEEK_API_KEY.strip():
        raise ValueError("DEEPSEEK_API_KEY 为空，请直接在代码里填入你的 key。")
...
def call_deepseek(prompt: str) -> str:
    ...
    if not choices:
        raise ValueError(f"No choices returned. Full response: {data}")
    ...
    if not result:
        raise ValueError(f"Empty content returned. Full response: {data}")

    return result

def call_with_retry(prompt: str, code: str, fail_log_file: str) -> str:
    for attempt in range(1, MAX_RETRY + 1):
        try:
            return call_deepseek(prompt=prompt)
        except Exception as e:
...
def process_drug_descriptions():
    ...
    with open(DRUG_OUTPUT_CSV, "a", newline="", encoding="utf-8") as f:
        ...
        for atc_code in tqdm(pending, desc="Generating drug descriptions"):
            ...
            prompt = DRUG_PROMPT_TMPL.format(atc_code=atc_code, ndc_code=ndc_code)
...
def main():
    ...
    print("====================================================")
    print("Starting generation pipeline with DeepSeek Chat")
    print("====================================================")
