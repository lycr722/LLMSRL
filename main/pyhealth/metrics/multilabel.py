...
def multilabel_metrics_fn(
    ...
    for metric in metrics:
        ...
        elif metric == "ddi_score":
            ddi_adj = np.load(os.path.join(CACHE_PATH, 'ddi_adj.npy'))
            y_pred_temp = [np.where(item)[0] for item in y_pred]
            output["ddi_score"] = ddi_rate_score(y_pred_temp, ddi_adj)
        elif metric == "avg_med":
            med_num = np.sum(y_pred, axis=1)
            avg_med_num = np.mean(med_num)
            output["avg_med"] = avg_med_num
